...
import { formatarDecimais } from './modules/utilities.js';

// companies and roles using the salaryData module.
...
function updateResults(){
  ...
  document.getElementById('salarySelected').innerText = `The salary for ${role}s at ${company} is \$${formatarDecimais(salary)}`;
  document.getElementById('salaryAverageByRole').innerText = `The industry average salary for ${role} positions is \$${formatarDecimais(averageSalaryByRole)}`;
  document.getElementById('salaryAverageByCompany').innerText = `The average salary at ${company} is \$${formatarDecimais(averageSalaryByCompany)}`;
  document.getElementById('salaryAverageIndustry').innerText = `The average salary in the Tech industry is \$${formatarDecimais(industryAverageSalary)}`;
}
